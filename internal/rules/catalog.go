...
var catalog = map[string]Spec{
	...
	"agent.config.unreadable": {
		ID:              "agent.config.unreadable",
		DefaultSeverity: model.SeverityLow,
		Confidence:      0.9,
		Description:     "Agent tooling configuration could not be parsed.",
		Rationale:       "Guard cannot audit MCP servers or plugin settings hidden behind invalid configuration syntax.",
		Evidence:        "A Claude or MCP settings file failed JSON parsing.",
		Remediation:     "Fix the agent configuration syntax and rerun guard agent audit.",
	},
	"agent.mcp.remote_bootstrap": {
		ID:              "agent.mcp.remote_bootstrap",
		DefaultSeverity: model.SeverityHigh,
		Confidence:      0.95,
		Description:     "Registered MCP executes remote bootstrap code.",
		Rationale:       "MCP servers extend agent capabilities and should not download and execute unreviewed code inline.",
		Evidence:        "The MCP command contains a curl/wget-to-shell pattern.",
		Remediation:     "Inspect the script first or replace the MCP with a pinned local binary/package.",
	},
	"agent.mcp.unpinned_npx": {
		ID:              "agent.mcp.unpinned_npx",
		DefaultSeverity: model.SeverityHigh,
		Confidence:      0.9,
		Description:     "Registered MCP uses an unpinned package runner.",
		Rationale:       "Unpinned npx/uvx/bunx commands can resolve different code over time.",
		Evidence:        "The MCP command uses a package runner without a pinned package version.",
		Remediation:     "Pin the MCP package version or install a trusted local binary.",
	},
	"agent.mcp.filesystem_broad": {
		ID:              "agent.mcp.filesystem_broad",
		DefaultSeverity: model.SeverityHigh,
		Confidence:      0.88,
		Description:     "Registered MCP exposes a broad filesystem scope.",
		Rationale:       "Filesystem MCPs with root or home access give the agent a large local blast radius.",
		Evidence:        "A filesystem MCP is configured with root or home directory access.",
		Remediation:     "Restrict the MCP filesystem scope to the specific project directories it needs.",
	},
	"agent.mcp.inline_secret_env": {
		ID:              "agent.mcp.inline_secret_env",
		DefaultSeverity: model.SeverityHigh,
		Confidence:      0.86,
		Description:     "Registered MCP contains an inline secret-looking environment value.",
		Rationale:       "Secrets embedded directly in agent configuration are easy to leak through logs, prompts, or repository files.",
		Evidence:        "An MCP env key looks sensitive and its value is literal rather than an environment reference.",
		Remediation:     "Move the value to an environment variable or secret manager reference.",
	},
	"agent.skill.remote_bootstrap": {
		ID:              "agent.skill.remote_bootstrap",
		DefaultSeverity: model.SeverityHigh,
		Confidence:      0.86,
		Description:     "Skill contains remote bootstrap instructions.",
		Rationale:       "Skills influence agent behavior and should not instruct inline execution of remote code.",
		Evidence:        "A SKILL.md file contains a curl/wget-to-shell pattern.",
		Remediation:     "Inspect the skill source and replace inline bootstrap commands with pinned, reviewable install steps.",
	},
	"agent.plugin.remote_bootstrap": {
		ID:              "agent.plugin.remote_bootstrap",
		DefaultSeverity: model.SeverityHigh,
		Confidence:      0.92,
		Description:     "Plugin hook contains remote bootstrap execution.",
		Rationale:       "Plugin hooks can run automatically during agent sessions and should not execute unreviewed remote code inline.",
		Evidence:        "A plugin hooks.json file contains a curl/wget-to-shell command.",
		Remediation:     "Disable or remove the plugin until the hook command is inspected and replaced with a pinned local command.",
	},
